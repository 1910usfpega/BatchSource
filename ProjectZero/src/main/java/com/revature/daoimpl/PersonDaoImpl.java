...
public class PersonDaoImpl implements PersonDao {
	...
	public void insertPerson(Person p) throws SQLException{
		...
		String sql = "insert into person values (nextval(\'user_seq\'), ?, ?,?,?)";
	...
	public ArrayList<Person> getAllPersons() throws SQLException {
		...
		try {
			...
			Person p = null;
			while(rs.next()) {
				p =new Person(rs.getString(2),rs.getString(3), rs.getString(4),rs.getString(5));
				personList.add(p);
			}return personList;
...
//	 }
	public boolean checkUser(String un, String pw) {
		Connection conn = cf.getConnection();
		 String str = "select all from person where username = ?";
		 try {
			 PreparedStatement ps = conn.prepareStatement(str);
			 ps.setString(1, un);
			 ResultSet rs = ps.executeQuery();
